...
def predict(archive_folder, test_file, output_file, cuda_device):
    ...
    with open(output_file, "w") as f:
        ...
        for batch in tqdm(iterator):
            ...
            output_embedding = model.embedding_forward(batch['text'], batch['metadata'])
            ...
            for i in range(len(para_ids)):
                ...
                res["words"] = words[i]
                res["prediction"] = [{"span": k, "label": v.split('_')[1] + '_False'} for k, v in predicted_ner[i].items()]
                res["gold"] = [{"span": k, "label": v.split('_')[1] + '_False'} for k, v in gold_ner[i].items()]
