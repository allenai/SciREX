...
from scirex.data.utils.section_feature_extraction import *
from scripts.entity_utils import *
from scirex.data.utils.span_utils import *
...
def clean_json_dict(json_dict):
    ...
    entities: Dict[Span, str] = OrderedDict([((s, e), t) for s, e, t in entities])

    corefs_all: Dict[str, List[Span]] = {
        cluster_name: sorted(list(set([tuple(x) for x in spans]))) for cluster_name, spans in json_dict["coref"].items()
    }

    n_ary_relations_all: List[Dict[BaseEntityType, str]] = [x for x in json_dict["n_ary_relations"]]

    cluster_to_type: Dict[str, BaseEntityType] = {}
    ...
    if "method_subrelations" in json_dict:
        ...
        method_subrelations: Dict[str, Set[str]] = {
    ...
    for e in entities:
        entities[e]: EntityType = (entities[e], str(any(e in v for v in corefs.values())))

    # Keep only relations where all clusters are non empty
    n_ary_relations = [r for r in n_ary_relations_all]
    ...
    for e in entities:
        in_sentences = [i for i, s in enumerate(json_dict["sentences"]) if is_x_in_y(e, s)] #Check entity lie in one sentence
...
class ScirexFullReader(DatasetReader):
        ...
        for dataset_path in datasets.values():
            with open(dataset_path, "r") as g:
                for idx, line in enumerate(g):
                    ...
                    corefs_non_salient: Dict[str, List[Span]] = {}  # json_dict.get("coref_non_salient", {})
                    ...
                    span_to_non_salient_cluster_ids: Dict[Span, List[int]] = {}
    ...
    def extract_coref_features(self, entities, words, span_to_cluster_ids):
        ...
        for i, j in combinations(range(len(entity_keys)), 2):
            ...
            c1, c2 = (
                set(span_to_cluster_ids.get((e1[0], e1[1]), [])),
                set(span_to_cluster_ids.get((e2[0], e2[1]), [])),
            )
    ...
    def resize_sections_and_group(self, sections: List[Span], sentences:List[List[Span]], entities: Dict[Span, EntityType]):
        ...
        # try :
        #     broken_sections = break_and_collapse_sections(sentences, min_len=20, max_len=self._max_paragraph_length)
        # except :
        #     breakpoint()
        ...
        # Group entities into sections they belong to
        for e in entities:
            is_in_n_para = 0
    ...
    def text_to_instance(
        ...
        fields = dict(text=text_field, ner_entity_labels=ner_entity_field, metadata=metadata_field)
        ...
        if len(spans) > 0:
            ...
            fields["span_saliency_labels"] = SequenceLabelField([0], fields["spans"], label_namespace="span_saliency_labels")
            fields["span_entity_labels"] = SequenceLabelField(
